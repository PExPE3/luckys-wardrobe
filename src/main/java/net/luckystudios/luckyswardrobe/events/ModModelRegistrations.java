...
public class ModModelRegistrations {
    ...
    public static void registerModelWithSwing(RegisterClientExtensionsEvent event, Item item, ModelLayerLocation layer, boolean isHead, int restAngle) {
        event.registerItem(new IClientItemExtensions() {
            @Override
            @OnlyIn(Dist.CLIENT)
            public HumanoidModel<?> getHumanoidArmorModel(LivingEntity living, ItemStack stack, EquipmentSlot slot, HumanoidModel<?> defaultModel) {
    ...
    // A very complex method, definitely can use some clean up.
    public static void registerDyeableModelWithSwing (RegisterClientExtensionsEvent event, Item item, ModelLayerLocation layer, boolean isHead, int restAngle, int defaultColor) {
        event.registerItem(new IClientItemExtensions() {
