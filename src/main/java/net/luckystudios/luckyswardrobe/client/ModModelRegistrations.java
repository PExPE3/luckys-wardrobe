package net.luckystudios.luckyswardrobe.client;
...
public class ModModelRegistrations {
    ...
    public static void registerCosmeticModel(RegisterClientExtensionsEvent event, Item item, ModelLayerLocation layer, int defaultColor) {
    ...
    public static void registerModelWithSwing (RegisterClientExtensionsEvent event, Item item, ModelLayerLocation layer, boolean isHead, int restAngle, int defaultColor) {
        event.registerItem(new IClientItemExtensions() {
            ...
            public void setupModelAnimations(LivingEntity entity, ItemStack stack, EquipmentSlot slot, Model model, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
                if (entity instanceof AbstractClientPlayer abstractClientPlayer) {
                    if (model instanceof HumanoidModel<?> humanoidModel) {
                        ...
                        float degrees = restAngle + forwardMotion / 2.0F + verticalMotion;
